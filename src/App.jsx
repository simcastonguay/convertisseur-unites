...
import { loadHistory, appendHistory, saveHistory } from './history.js';
...
function Icon({ name, size = 24 }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.65" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
    {name === 'temperature' ? <><path d="M9 14V5a3 3 0 0 1 6 0v9a5 5 0 1 1-6 0Z"/><path d="M12 7v10m5-10h3m-3 4h2"/><circle cx="12" cy="18" r="1"/></> : name === 'volume' ? <><path d="M12 3C9 7 5 11 5 15a7 7 0 0 0 14 0c0-4-4-8-7-12Z"/><path d="M9 15a3 3 0 0 0 3 3"/></> : name === 'speed' ? <><path d="M4 17a8 8 0 1 1 16 0"/><path d="m12 17 4-5"/><path d="M7 13h1m8 0h1M12 9v1"/></> : name === 'swap' ? <><path d="M4 7h16m-4-4 4 4-4 4M20 17H4m4-4-4 4 4 4"/></> : <><path d="m3 16 13-13 5 5L8 21Z"/><path d="m7 12 2 2m2-6 2 2m2-6 2 2"/></>}
  </svg>;
}
...
export default function App() {
  ...
  useEffect(() => {
    try { setStorageUnavailable(!saveHistory(window.localStorage, history)); }
  ...
  function restoreHistory(entry) {
    setCategoryId(entry.category); setFrom(entry.from); setTo(entry.to); setValue(String(entry.value)); clearResult();
  }
  async function submit(event) {
    ...
    try {
      ...
      if (version === requestVersion.current) {
        setResult(data);
        setHistory((previous) => appendHistory(previous, { category: data.category, value: data.value, result: data.result, from: data.from, to: data.to, fromSymbol: source.symbol, toSymbol: target.symbol }));
      }
    } catch (error) {
  ...
  return <div className="app-shell">
      ...
      {loadError ? <Alert severity="error">{loadError}</Alert> : !category ? <div className="loading"><CircularProgress size={28} aria-label="Chargement des unités" /></div> : <>
        <div className="workspace">
          <Paper component="section" elevation={0} className="converter" aria-labelledby="converter-title">
            ...
            <section className={`result-panel ${result ? 'has-result' : ''}`} aria-live="polite" aria-atomic="true"><p className="result-label">VOTRE RÉSULTAT</p>{result ? <><p className="result-origin">{format(result.value)} {source.symbol} =</p><p className="result-number">{format(result.result)} <span>{target.symbol}</span></p><p className="result-formula">{result.category === 'temperature' ? <>{target.symbol} = {source.symbol} × {format(result.factor)} {result.offset < 0 ? '−' : '+'} {format(Math.abs(result.offset))}</> : <>1 {source.symbol} = {format(result.factor)} {target.symbol}</>}</p></> : <><p className="result-placeholder">Chaque mesure a son équivalent.</p><p className="result-hint">Votre résultat s’affichera ici.</p></>}</section>
          </Paper>
          <aside className="side-panel"><span className="aside-icon"><Icon name={categoryId} size={32}/></span><p className="eyebrow">LES UNITÉS, EN TOUTE SIMPLICITÉ</p><h2>Un petit calcul.<br/>Un grand coup de main.</h2><p>{category.description}</p><div className="ruler" aria-hidden="true"><div/><span>0</span><span>5</span><span>10</span></div><div className="tip"><span className="tip-label">LE SAVIEZ-VOUS ?</span><p>{categoryId === 'temperature' ? '0 °C correspond à 32 °F et à 273,15 K. Contrairement aux longueurs, il faut aussi ajouter un décalage pour convertir les températures.' : categoryId === 'speed' ? 'Un nœud vaut exactement 1,852 km/h, soit un mille marin à l’heure. C’est l’unité de la navigation maritime et aérienne.' : categoryId === 'volume' ? 'Un gallon américain et un gallon impérial n’ont pas le même volume. Ici, vous choisissez lequel utiliser.' : 'Un pied mesure exactement 0,3048 mètre. Une référence pratique pour vos projets et vos mesures.'}</p></div></aside>
        </div>
        <History entries={history} onRestore={restoreHistory} onClear={() => setHistory([])} format={format} storageUnavailable={storageUnavailable}/>
    ...
    </main><footer><span>unité. <span className="footer-muted">La mesure, tout simplement.</span></span><span>Résultats affichés à 10 chiffres significatifs</span></footer>
  </div>;
}
