    ...
    <main>
      ...
      {loadError ? <Alert severity="error">{loadError}</Alert> : !category ? <div className="loading"><CircularProgress size={28} aria-label="Chargement des unités" /></div> : <>
        <div className="workspace">
          ...
          <aside className="side-panel"><span className="aside-icon"><Icon name={categoryId} size={32}/></span><p className="eyebrow">LES UNITÉS, EN TOUTE SIMPLICITÉ</p><h2>Un petit calcul.<br/>Un grand coup de main.</h2><p>{category.description}</p><div className="ruler" aria-hidden="true"><div/><span>0</span><span>5</span><span>10</span></div><div className="tip"><span className="tip-label">LE SAVIEZ-VOUS ?</span><p>{categoryId === 'temperature' ? '0 °C correspond à 32 °F et à 273,15 K. Contrairement aux longueurs, il faut aussi ajouter un décalage pour convertir les températures.' : categoryId === 'volume' ? 'Un gallon américain et un gallon impérial n’ont pas le même volume. Ici, vous choisissez lequel utiliser.' : 'Un pied mesure exactement 0,3048 mètre. Une référence pratique pour vos projets et vos mesures.'}</p></div></aside>
        </div>
